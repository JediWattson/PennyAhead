...
export async function strandsReply(
  ...
  const agent = new Agent({
    model,
    messages: parseChatHistory(history).map((entry) => ({
      role: entry.role,
      content: [{ type: 'textBlock' as const, text: entry.text }],
    })),
    printer: false,
    tools: [
      readTool(
